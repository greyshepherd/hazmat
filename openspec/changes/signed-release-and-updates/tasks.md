...
- [ ] 8.1 Write the publish step: create the release, upload the disk image to it, then write the feed and push it, with the repository, the tag prefix and the feed's path read from the release configuration and the token from the environment. Verified so far: the step is written and takes every value from the configuration, it checks the archive answers at its address before it writes the entry, and the feed as served reports `cache-control: max-age=600`, ten minutes, which is inside the hour the channel requires. The ordering itself is unchecked until a real release is published
