...
`ShellWindow` (in `HazmatApp`) makes the window on `show(_:)`: 1080 × 700 by default, `NSHostingView` over `ShellView` with `sizingOptions = [.minSize]` for the content minimum and `sceneBridgingOptions = [.title, .toolbars]` so the navigation title, the toolbar and the sidebar search reach the window, a frame autosave name for size and position, `isReleasedWhenClosed = false`. On its `willCloseNotification` the controller drops the window, which releases the window, the hosting view and the view tree. `ShellModel.showWindow()` shows it, sets `windowShowing` and reads in full; the close observer clears `windowShowing`, resets the view mode and reads without detail. `paneGeneration`, the `.id` modifiers, `WindowReader`, `windowChanged(_:)` and the `didUpdateNotification` observer go. The `Window` scene goes; `.commands` attaches to the `Settings` scene; `StatusMenu` and `ShellCommands` call `showWindow()`; an `NSApplicationDelegateAdaptor` shows the window at launch as the scene did.
