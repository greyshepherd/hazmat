...
profile renders now, MUST report the difference, and MUST NOT overwrite a
differing block unless the caller asked for that explicitly.
...
- **WHEN** the caller asks to overwrite a drifted block
- **THEN** the apply replaces the block and reports that it overwrote drift
